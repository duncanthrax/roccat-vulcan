...
void rv_fx_topo_keys() {
	...
	unsigned char ev2rv[RV_MAX_EV_CODE];
	memset(ev2rv, 0xff, RV_MAX_EV_CODE);
	...
	for (int key = 0; key < RV_NUM_KEYS; key++) {
		...
		// Timed out
		map.key[key] = off;
		rv_printf(RV_LOG_NORMAL, "timed out\n");

		NEXT_KEY:
		key = key;
	}

	for (int code = 0; code <= RV_MAX_EV_CODE; code++) {
...
void rv_fx_topo_neigh() {
	...
	// rv_printf(RV_LOG_NORMAL, "Recording neighbor topology. Neighbors are keys that share part of an edge with the candidate key. Sharing a corner is NOT a neighbor.\n");
	...
	// 		if (rv_pressed_keys[k] == stop_key) goto EXIT;
